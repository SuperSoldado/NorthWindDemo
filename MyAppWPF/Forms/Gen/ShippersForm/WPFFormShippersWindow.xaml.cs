...
using MyAppWPFLib;
    ...
    public partial class FormWPFShippers : Page
    {    
        ...
        public FormWPFShippers(WPFConfig config)
        {
            this.config = config;
            dataConnection = new WPFShippersDB(config);
            string error = null;
            ShippersDataContext = dataConnection.GetEmptyDataContext(out error);
            if (error != null)
            {
        ...
        {    
            string error = null;
            var itemToSave = ShippersDataContext.modelNotifiedForShippersMain;
        ...
        {
            PropertyInfo[] sourceListClassInfoProperties = typeof(ModelNotifiedForShippers).GetProperties();
            foreach (PropertyInfo sourceProperty in sourceListClassInfoProperties)
            {
                sourceProperty.SetValue(ShippersDataContext.modelNotifiedForShippersMain, null, null);
            }
        ...
        private void btnDelete_Click(object sender, RoutedEventArgs e)
        {
            ...
            switch (result)
            {
                case MessageBoxResult.Yes:
                    dataConnection.DeleteData(ShippersDataContext.modelNotifiedForShippersMain, out error);
                    break;
