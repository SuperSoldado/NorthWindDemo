...
{
	public partial class TerritoriesBsn
    {
        ...
        /// </summary>
        public TerritoriesBsn(IBaseGlobalConfig baseGlobalConfig)
        {
            this.baseGlobalConfig = baseGlobalConfig;
            this.motor = new Motor(baseGlobalConfig.GetMainConnectionString);
            GenerateDataAcces();
            this.TerritoriesDAO = new TerritoriesDAO(motor);
        }
        ...
        public TerritoriesBsn(bool closeConnectionWhenFinish, Motor motor)
        {
            this.motor = motor;
            this.TerritoriesDAO = new TerritoriesDAO(motor);
        ...
        /// </summary>
        public TerritoriesDAO TerritoriesDAO = null;
        ...
        {
            motor.OpenConnection();
            List<TerritoriesInfo> list = TerritoriesDAO.GetAll(filterExpression);
            if (this.closeConnectionWhenFinish)
            {
                motor.CloseConnection();
            }
        ...
        {
            motor.OpenConnection();
            List<TerritoriesInfo> list = TerritoriesDAO.GetAll();
            if (this.closeConnectionWhenFinish)
            {
                motor.CloseConnection();
            }
        ...
        /// <returns></returns>
        public virtual List<TerritoriesInfo> GetAll(int numberOfRowsToSkip, int numberOfRows)
        {
            motor.OpenConnection();
            List<TerritoriesInfo> list = TerritoriesDAO.GetAll(numberOfRowsToSkip, numberOfRows);
            if (this.closeConnectionWhenFinish)
            {
                motor.CloseConnection();
            }
        ...
        /// </summary>
        /// <param name="TerritoriesInfo">Object to insert.</param>
        /// <param name="errorMessage">Error message if exception is throwed</param>
        public virtual void InsertOne(TerritoriesInfo parTerritoriesInfo, out string errorMessage)
        {
            ...
            this.InsertOne(parTerritoriesInfo, transaction, out errorMessage);
...
parTerritoriesInfo.RegionID = fkClass.RegionID;
            ...
            TerritoriesDAO.InsertOne(parTerritoriesInfo, transaction, out errorMessage);
