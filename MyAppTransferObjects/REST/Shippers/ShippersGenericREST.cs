...
namespace MyApp.TransferObjects.REST
{
    ...
    /// </summary>
    public class ShippersGenericREST : GenericRESTBase
    {
        private WPFConfig wpfConfig { get; set; }
        public ShippersGenericREST(WPFConfig wpfConfig)
        {
            this.wpfConfig = wpfConfig;
        }

        public void Delete(DeleteShippersView deleteData, out string error)
        {
            error = null;
            try
            {
                Uri basePath = new Uri(wpfConfig.RESTBasePath);
                Uri requestUri = new Uri(basePath, "Shippers/Delete");
        ...
        public void Insert(CreateShippersView insertData, out string error)
        {
            error = null;
            try
            {
                Uri basePath = new Uri(wpfConfig.RESTBasePath);
                Uri requestUri = new Uri(basePath, "Shippers/Create");
                GeneralBodyPost postRequest = new GeneralBodyPost();
        ...
        public void Update(UpdateShippersView updateData, out string error)
        {
            error = null;
            try
            {
                Uri basePath = new Uri(wpfConfig.RESTBasePath);
                Uri requestUri = new Uri(basePath, "Shippers/Update");
                GeneralBodyPost postRequest = new GeneralBodyPost();
        ...
        public List<T> GetByPK<T>(int ShipperID, out string error)
        {
            ...
            dataFilterExpressionREST.FieldName= "ShipperID";
            dataFilterExpressionREST.FilterType= DataFilterExpressionREST._FilterType.Equal;
            dataFilterExpressionREST.Filter = ShipperID.ToString();
        ...
        public List<T> GetAllX<T>(List<DataFilterExpressionREST> dataFilterExpressionRESTs, out string error)
        {
            ...
            {
                Uri basePath = new Uri(wpfConfig.RESTBasePath);
                Uri requestUri = new Uri(basePath, "Shippers/GetAllX");
                ...
                using (var client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        List<GetShippersView> dataRetrieved = jArray.ToObject<List<GetShippersView>>();
                        List<T> result = new List<T>();
                        foreach (GetShippersView item in dataRetrieved)
                        {
                            T modelNotifiedForShippersNew = (T)Activator.CreateInstance(typeof(T));
                            Cloner.CopyAllTo(typeof(GetShippersView), item, typeof(T), modelNotifiedForShippersNew);
                            result.Add(modelNotifiedForShippersNew);
                        }
                        return result;
                    }
                    else
        ...
        public List<T> GetAll<T>(int numberOfItens, int itensToSkip, out string error)
        {
            ...
            {
                Uri basePath = new Uri(wpfConfig.RESTBasePath);
                Uri requestUri = new Uri(basePath, "Shippers/GetAll");
                GeneralBodyGet getRequest = new GeneralBodyGet();
