...
namespace MyApp.TransferObjects.REST
{
    ...
    /// </summary>
    public class CategoriesGenericREST : GenericRESTBase
    {
        private WPFConfig wpfConfig { get; set; }
        public CategoriesGenericREST(WPFConfig wpfConfig)
        {
            this.wpfConfig = wpfConfig;
        }

        public void Delete(DeleteCategoriesView deleteData, out string error)
        {
            error = null;
            try
            {
                Uri basePath = new Uri(wpfConfig.RESTBasePath);
                Uri requestUri = new Uri(basePath, "Categories/Delete");
        ...
        public void Insert(CreateCategoriesView insertData, out string error)
        {
            error = null;
            try
            {
                Uri basePath = new Uri(wpfConfig.RESTBasePath);
                Uri requestUri = new Uri(basePath, "Categories/Create");
                GeneralBodyPost postRequest = new GeneralBodyPost();
        ...
        public void Update(UpdateCategoriesView updateData, out string error)
        {
            error = null;
            try
            {
                Uri basePath = new Uri(wpfConfig.RESTBasePath);
                Uri requestUri = new Uri(basePath, "Categories/Update");
                GeneralBodyPost postRequest = new GeneralBodyPost();
        ...
        public List<T> GetByPK<T>(int CategoryID, out string error)
        {
            ...
            dataFilterExpressionREST.FieldName= "CategoryID";
            dataFilterExpressionREST.FilterType= DataFilterExpressionREST._FilterType.Equal;
            dataFilterExpressionREST.Filter = CategoryID.ToString();
        ...
        public List<T> GetAllX<T>(List<DataFilterExpressionREST> dataFilterExpressionRESTs, out string error)
        {
            ...
            {
                Uri basePath = new Uri(wpfConfig.RESTBasePath);
                Uri requestUri = new Uri(basePath, "Categories/GetAllX");
                ...
                using (var client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        List<GetCategoriesView> dataRetrieved = jArray.ToObject<List<GetCategoriesView>>();
                        List<T> result = new List<T>();
                        foreach (GetCategoriesView item in dataRetrieved)
                        {
                            T modelNotifiedForCategoriesNew = (T)Activator.CreateInstance(typeof(T));
                            Cloner.CopyAllTo(typeof(GetCategoriesView), item, typeof(T), modelNotifiedForCategoriesNew);
                            result.Add(modelNotifiedForCategoriesNew);
                        }
                        return result;
                    }
                    else
        ...
        public List<T> GetAll<T>(int numberOfItens, int itensToSkip, out string error)
        {
            ...
            {
                Uri basePath = new Uri(wpfConfig.RESTBasePath);
                Uri requestUri = new Uri(basePath, "Categories/GetAll");
                GeneralBodyGet getRequest = new GeneralBodyGet();
