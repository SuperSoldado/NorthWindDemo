...
namespace MyApp.TransferObjects.REST
{
    ...
    /// </summary>
    public class OrderDetailsGenericREST : GenericRESTBase
    {
        private WPFConfig wpfConfig { get; set; }
        public OrderDetailsGenericREST(WPFConfig wpfConfig)
        {
            this.wpfConfig = wpfConfig;
        }

        public void Delete(DeleteOrderDetailsView deleteData, out string error)
        {
            error = null;
            try
            {
                Uri basePath = new Uri(wpfConfig.RESTBasePath);
                Uri requestUri = new Uri(basePath, "OrderDetails/Delete");
        ...
        public void Insert(CreateOrderDetailsView insertData, out string error)
        {
            error = null;
            try
            {
                Uri basePath = new Uri(wpfConfig.RESTBasePath);
                Uri requestUri = new Uri(basePath, "OrderDetails/Create");
                GeneralBodyPost postRequest = new GeneralBodyPost();
        ...
        public void Update(UpdateOrderDetailsView updateData, out string error)
        {
            error = null;
            try
            {
                Uri basePath = new Uri(wpfConfig.RESTBasePath);
                Uri requestUri = new Uri(basePath, "OrderDetails/Update");
                GeneralBodyPost postRequest = new GeneralBodyPost();
        ...
        public List<T> GetAllX<T>(List<DataFilterExpressionREST> dataFilterExpressionRESTs, out string error)
        {
            ...
            {
                Uri basePath = new Uri(wpfConfig.RESTBasePath);
                Uri requestUri = new Uri(basePath, "OrderDetails/GetAllX");
                ...
                using (var client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        List<GetOrderDetailsView> dataRetrieved = jArray.ToObject<List<GetOrderDetailsView>>();
                        List<T> result = new List<T>();
                        foreach (GetOrderDetailsView item in dataRetrieved)
                        {
                            T modelNotifiedForOrderDetailsNew = (T)Activator.CreateInstance(typeof(T));
                            Cloner.CopyAllTo(typeof(GetOrderDetailsView), item, typeof(T), modelNotifiedForOrderDetailsNew);
                            result.Add(modelNotifiedForOrderDetailsNew);
                        }
                        return result;
                    }
                    else
        ...
        public List<T> GetAll<T>(int numberOfItens, int itensToSkip, out string error)
        {
            ...
            {
                Uri basePath = new Uri(wpfConfig.RESTBasePath);
                Uri requestUri = new Uri(basePath, "OrderDetails/GetAll");
                GeneralBodyGet getRequest = new GeneralBodyGet();
