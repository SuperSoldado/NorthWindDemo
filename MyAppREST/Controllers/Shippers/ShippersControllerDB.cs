...
{
    public class RESTShippersDB : RESTBaseDB
    {
        ...
        public void TryDelete(DeleteShippersView viewToDelete, out RestExceptionError error)
        {
            error = null;
            ShippersInfo dbViewToDelete = new ShippersInfo();
            try
            {
                Cloner.CopyAllTo(typeof(DeleteShippersView), viewToDelete, typeof(ShippersInfo), dbViewToDelete);
            }
            catch (Exception ex)
            {
                error = new RestExceptionError();
                error.InternalMessage = "Internal Error parsing data for (Shippers.TryDelete/Parsing)";
            ...
            try
            {
                ...
                {
                    error = new RestExceptionError();
                    error.InternalMessage = "Internal Error Save data for [Shippers.TryDelete]";
            ...
            {
                error = new RestExceptionError();
                error.InternalMessage = "Internal Error Update data for [Shippers.TryDelete]";
                error.ExceptionMessage = ex.Message;
                error.SourceError = RestExceptionError._SourceError.ServerSide;
                error.StackTrace = ex.StackTrace;
            }
        ...
        public void TryInclude(CreateShippersView viewToInclude, out RestExceptionError error)
        {
            error = null;
            ShippersInfo dbViewToInclude = new ShippersInfo();
            try
            {                
                Cloner.CopyAllTo(typeof(CreateShippersView), viewToInclude, typeof(ShippersInfo), dbViewToInclude);                
            }
            catch (Exception ex)
            {
                error = new RestExceptionError();
                error.InternalMessage = "Internal Error parsing data for (Shippers.TryInclude/Parsing)";
                error.ExceptionMessage = ex.Message;
                error.SourceError = RestExceptionError._SourceError.ServerSide;
                error.StackTrace = ex.StackTrace;
            }
            ...
            try
            {
                ...
                {
                    error = new RestExceptionError();
                    error.InternalMessage = "Internal Error Save data for [Shippers.TryInclude/Save]";
                    error.ExceptionMessage = dbError;
                    error.SourceError = RestExceptionError._SourceError.ServerSide;
                    error.StackTrace = "";
                }
        ...
        public T TryParse<T>(GeneralBodyPost generalBodyPost, out RestExceptionError error)
        {
            ...
            {
                error = new RestExceptionError();
                error.InternalMessage = "No data from client to include [Shippers.TryParse]";
                error.ExceptionMessage = "No data";
                error.SourceError = RestExceptionError._SourceError.ClientSide;
                error.StackTrace = "RESTShippersDB.TryParse";
                return default(T);
            }
            ...
            catch (Exception ex)
            {
                ...
                error.ExceptionMessage = ex.Message;
                error.SourceError = RestExceptionError._SourceError.ClientSide;
                error.InternalMessage = "Error parsing data from body to view for [Shippers.TryParse]";
                return default(T);
            }            
        }
        ...
        /// </summary>
        public List<GetShippersView> Filter(List<GetShippersView> rawList, GeneralBodyGet GeneralBodyGet, out RestExceptionError error)
        {
            try
            {
                error = null;
                ...
                List<GetShippersView> result = null;
                if (linqQuery != null)
                {
                ...
                else
                {
                    ...
                    result = filteredList.Cast<GetShippersView>().ToList();
                }
        ...
        /// Examples:
        /// "Where(GetAllShippersView => (GetAllShippersView.ShippersID = 4))"
        /// </summary>
        /// <param name="list"></param>
        /// <param name="GeneralBodyGet"></param>
        /// <param name="error"></param>
        /// <returns></returns>
        public List<GetShippersView> DynamicLinqQuery(List<GetShippersView> list, string linqQuery, out RestExceptionError error)
        {
            try
            {
                error = null;

                IQueryable<GetShippersView> queryableData = list.AsQueryable<GetShippersView>();
                var externals = new Dictionary<string, object>();
                externals.Add("list", queryableData);
                string query = "list." + linqQuery;//Example: "list.Where(GetShippersView => (GetShippersView.ShippersID = 4))";
                var expression = System.Linq.Dynamic.DynamicExpression.Parse(typeof(IQueryable<GetShippersView>), query, new[] { externals });
                var result = queryableData.Provider.CreateQuery<GetShippersView>(expression);
        ...
        public List<GetShippersView> OrderByAndTrim(List<GetShippersView> list, GeneralBodyGet GeneralBodyGet, out RestExceptionError error)
        {
            try
            {
                error = null;
                ...
                List<GetShippersView> filteredList = null;
                //1)Ordering
