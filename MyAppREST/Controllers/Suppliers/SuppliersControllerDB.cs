...
{
    public class RESTSuppliersDB : RESTBaseDB
    {
        ...
        public void TryDelete(DeleteSuppliersView viewToDelete, out RestExceptionError error)
        {
            error = null;
            SuppliersInfo dbViewToDelete = new SuppliersInfo();
            try
            {
                Cloner.CopyAllTo(typeof(DeleteSuppliersView), viewToDelete, typeof(SuppliersInfo), dbViewToDelete);
            }
            catch (Exception ex)
            {
                error = new RestExceptionError();
                error.InternalMessage = "Internal Error parsing data for (Suppliers.TryDelete/Parsing)";
            ...
            try
            {
                ...
                {
                    error = new RestExceptionError();
                    error.InternalMessage = "Internal Error Save data for [Suppliers.TryDelete]";
            ...
            {
                error = new RestExceptionError();
                error.InternalMessage = "Internal Error Update data for [Suppliers.TryDelete]";
                error.ExceptionMessage = ex.Message;
                error.SourceError = RestExceptionError._SourceError.ServerSide;
                error.StackTrace = ex.StackTrace;
            }
        ...
        public void TryInclude(CreateSuppliersView viewToInclude, out RestExceptionError error)
        {
            error = null;
            SuppliersInfo dbViewToInclude = new SuppliersInfo();
            try
            {                
                Cloner.CopyAllTo(typeof(CreateSuppliersView), viewToInclude, typeof(SuppliersInfo), dbViewToInclude);                
            }
            catch (Exception ex)
            {
                error = new RestExceptionError();
                error.InternalMessage = "Internal Error parsing data for (Suppliers.TryInclude/Parsing)";
                error.ExceptionMessage = ex.Message;
                error.SourceError = RestExceptionError._SourceError.ServerSide;
                error.StackTrace = ex.StackTrace;
            }
            ...
            try
            {
                ...
                {
                    error = new RestExceptionError();
                    error.InternalMessage = "Internal Error Save data for [Suppliers.TryInclude/Save]";
                    error.ExceptionMessage = dbError;
                    error.SourceError = RestExceptionError._SourceError.ServerSide;
                    error.StackTrace = "";
                }
        ...
        public T TryParse<T>(GeneralBodyPost generalBodyPost, out RestExceptionError error)
        {
            ...
            {
                error = new RestExceptionError();
                error.InternalMessage = "No data from client to include [Suppliers.TryParse]";
                error.ExceptionMessage = "No data";
                error.SourceError = RestExceptionError._SourceError.ClientSide;
                error.StackTrace = "RESTSuppliersDB.TryParse";
                return default(T);
            }
            ...
            catch (Exception ex)
            {
                ...
                error.ExceptionMessage = ex.Message;
                error.SourceError = RestExceptionError._SourceError.ClientSide;
                error.InternalMessage = "Error parsing data from body to view for [Suppliers.TryParse]";
                return default(T);
            }            
        }
        ...
        /// </summary>
        public List<GetSuppliersView> Filter(List<GetSuppliersView> rawList, GeneralBodyGet GeneralBodyGet, out RestExceptionError error)
        {
            try
            {
                error = null;
                ...
                List<GetSuppliersView> result = null;
                if (linqQuery != null)
                {
                ...
                else
                {
                    ...
                    result = filteredList.Cast<GetSuppliersView>().ToList();
                }
        ...
        /// Examples:
        /// "Where(GetAllSuppliersView => (GetAllSuppliersView.SuppliersID = 4))"
        /// </summary>
        /// <param name="list"></param>
        /// <param name="GeneralBodyGet"></param>
        /// <param name="error"></param>
        /// <returns></returns>
        public List<GetSuppliersView> DynamicLinqQuery(List<GetSuppliersView> list, string linqQuery, out RestExceptionError error)
        {
            try
            {
                error = null;

                IQueryable<GetSuppliersView> queryableData = list.AsQueryable<GetSuppliersView>();
                var externals = new Dictionary<string, object>();
                externals.Add("list", queryableData);
                string query = "list." + linqQuery;//Example: "list.Where(GetSuppliersView => (GetSuppliersView.SuppliersID = 4))";
                var expression = System.Linq.Dynamic.DynamicExpression.Parse(typeof(IQueryable<GetSuppliersView>), query, new[] { externals });
                var result = queryableData.Provider.CreateQuery<GetSuppliersView>(expression);
        ...
        public List<GetSuppliersView> OrderByAndTrim(List<GetSuppliersView> list, GeneralBodyGet GeneralBodyGet, out RestExceptionError error)
        {
            try
            {
                error = null;
                ...
                List<GetSuppliersView> filteredList = null;
                //1)Ordering
